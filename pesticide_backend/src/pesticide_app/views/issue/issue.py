...
class IssueViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        projectPageLink = "http://127.0.0.1:3000/projects/" + slugify(issue.project.name) 
    ...
    @action(
        ...
        permission_classes=[IsAuthenticated & (IssueProjectCreatorOrMembers | AdminOrReadOnlyPermisions)], 
        authentication_classes=[TokenAuthentication]
    )
    def update_issue_status(self, request, pk):
        ...
        if (user.is_master or user == issue.project.creator or user in issue.project.members.all()):
            try:
                new_status = IssueStatus.objects.get(id=self.request.data['status'])
            except:
                return Response(
            ...
            if old_status != new_status:
                ...
                email_notification = threading.Thread(
                    ...
                    args=(
                        ...
                        projectPageLink, 
                        issue.title, 
                        IssueStatus.objects.get(id=1),  # old_status temporarily set as IssueStatus of id = 1...
                        new_status,
                        status_updated_by,
                        issue.reporter,
                        issue.project.members.all()
