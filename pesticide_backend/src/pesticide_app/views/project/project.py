...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        projectPageLink = "http://127.0.0.1:3000/projects/" + slugify(project.name) 
        ...
        if (user.is_master or user == project.creator or user in project.members.all()):
            ...
            if set(old_member_list) != set(members):
                ...
                for i in range(len(new_member_list)):
                    new_member_list[i] = User.objects.get(id = new_member_list[i])
                members_to_be_added = list(set(new_member_list).difference(set(old_member_list)))
